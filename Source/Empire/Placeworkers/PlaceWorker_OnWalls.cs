...
namespace TheEndTimes_Empire
{
    public class PlaceWorker_OnWalls : PlaceWorker
    {
                                                              ...
                                                              Thing thing = null)
        {
            ...
            bool isMassiveCastleWall = building.def.defName.Contains("CastleWall");
            ...
            if (rot.FacingCell != IntVec3.Invalid)
            {
                ...
                if (!isMassiveCastleWall && (facingLocBuilding != null && facingLocBuilding.def != null && facingLocBuilding.def.graphicData != null && EmpireUtil.IsWallRockDoorOrSolid(facingLocBuilding)))
                    return (AcceptanceReport)"RH_TET_Empire_OpenSpaceNeeded".Translate();
            }
        ...
        public override void DrawGhost(
          ...
          Thing _thing = null)
        {
            if (def.defName.Equals("RH_TET_Empire_GreatStagMount"))
                return;
                    ...
                    using (List<IntVec3>.Enumerator enumerator1 = new List<IntVec3>(roomGroup.Cells).GetEnumerator())
                    {
                        while (enumerator1.MoveNext())
                        {
                            ...
                            using (List<Thing>.Enumerator enumerator2 = GridsUtility.GetThingList(current, currentMap).GetEnumerator())
                            {
                                ...
                                {
                                    if (((string)((Def)enumerator2.Current.def).defName).Contains("Empire_WallLightGlower"))
                                    {
